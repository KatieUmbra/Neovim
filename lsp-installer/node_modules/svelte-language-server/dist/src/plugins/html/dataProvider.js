...
const svelteEvents = [
    ...(webCustomData_1.htmlData.globalAttributes?.filter(isEvent).map(mapToSvelteEvent) ?? []),
    {
        name: 'on:introstart',
...
const sveltekitAttributes = [
    {
        name: 'data-sveltekit-keepfocus',
        description: 'SvelteKit-specific attribute. Currently focused element will retain focus after navigation. Otherwise, focus will be reset to the body.',
        valueSet: 'v'
    },
    {
    ...
    },
    {
        ...
        description: 'SvelteKit-specific attribute. Will replace the current `history` entry rather than creating a new one with `pushState` when the link is clicked.',
        valueSet: 'v'
    }
];
const svelteTags = [
    ...
    },
    {
        ...
        description: "As with <svelte:window>, this element allows you to add listeners to events on document, such as visibilitychange, which don't fire on window.",
        attributes: [
            {
...
const addAttributes = {
    ...
    script: [
        {
            name: 'generics',
            description: 'Generics used within the components. Only available when using TypeScript.'
        }
    ]
};
...
exports.svelteHtmlDataProvider = (0, vscode_html_languageservice_1.newHTMLDataProvider)('svelte-builtin', {
    version: 1,
    globalAttributes: [
        ...webCustomData_1.htmlData.globalAttributes,
        ...svelteEvents,
        ...svelteAttributes,
        ...sveltekitAttributes
    ],
    ...
    valueSets: webCustomData_1.htmlData.valueSets?.map((set) => ({
        name: set.name,
        values: (0, utils_1.unique)(set.values)
    })) ?? []
});
